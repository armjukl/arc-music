...
export function MobilePlayer({
  ...
  return (
    <>
      <div
        className={clsx(
          "md:hidden fixed bottom-0 left-0 right-0 h-20 z-30 bg-white/80 backdrop-blur border-t border-slate-200 dark:bg-slate-900/80 dark:border-slate-700 flex items-center px-3 transform transition-transform duration-300 ease-in-out",
      ...
      <div
        className={clsx(
          "md:hidden fixed inset-x-0 bottom-0 z-40 bg-white/90 backdrop-blur shadow-2xl flex flex-col overflow-hidden rounded-t-2xl transform transition-transform duration-300 ease-in-out dark:bg-slate-900/90",
          mobileExpanded
        ...
        <div className="flex-1 flex flex-col p-4">
          <div className="flex-1 w-full flex flex-col">
            {(hasAnyLyric || currentSong?.publishedAt) ? <div className={clsx("w-full", lyricsExpanded && "flex-1 flex flex-col")}>
              ...
              {lyricsExpanded ? <div ref={lyricContainerRef} className="flex-1 min-h-[8rem] overflow-y-auto max-h-[calc(100vh-24rem)] custom-scrollbar bg-white/70 border border-slate-200 rounded-xl p-3 dark:bg-slate-800/70 dark:border-slate-700">
